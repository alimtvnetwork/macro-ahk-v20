...
function readCollapsedState(storageKey: string): boolean {
  ...
  try { savedState = localStorage.getItem(storageKey); } catch (_e) { console.debug('[RiseupAsia] [readCollapsedState] localStorage read failed for ' + storageKey); }
...
function persistCollapsedState(storageKey: string, isExpanding: boolean): void {
  try { localStorage.setItem(storageKey, isExpanding ? 'expanded' : 'collapsed'); } catch (_e) { console.debug('[RiseupAsia] [persistCollapsedState] localStorage write failed for ' + storageKey); }
}
