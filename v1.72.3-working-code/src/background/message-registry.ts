...
export const HANDLER_REGISTRY = new Map<MessageType, MessageHandler>([
    ...
    [MessageType.GET_RECENT_MESSAGES, async (msg) => {
        const limit = (msg as any).limit ?? 10;
    ...
    [MessageType.GET_UPDATER, async (msg) => ({ updater: handleGetUpdater((msg as any).updaterId) })],
    [MessageType.CREATE_UPDATER, async (msg) => ({ updaterId: handleCreateUpdater((msg as any).data) })],
    [MessageType.DELETE_UPDATER, async (msg) => { handleDeleteUpdater((msg as any).updaterId); return { isOk: true }; }],
    [MessageType.CHECK_FOR_UPDATE, async (msg) => handleCheckForUpdate((msg as any).updaterId)],
    [MessageType.GET_UPDATE_SETTINGS, async () => ({ settings: handleGetUpdateSettings() })],
    [MessageType.SAVE_UPDATE_SETTINGS, async (msg) => { handleSaveUpdateSettings((msg as any).data); return { isOk: true }; }],
    // ─── SDK Bridge (marco.*) ───
