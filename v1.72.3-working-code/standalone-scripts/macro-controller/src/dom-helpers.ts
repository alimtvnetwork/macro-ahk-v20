...
import {
  getByXPath, getAllByXPath, findElement, reactClick, ML_ELEMENTS,
} from './xpath-utils';
 ...
 * If so, the loop should NOT open the project dialog (disrupts typing).
 */
 ...
 * Calls back immediately when main bar appears.
 */
export function pollForDialogReady(callback?: Function): void {
  ...
  if (!mainXpath || mainXpath.indexOf('__') === 0) {
    ...
    setTimeout(callback as TimerHandler, TIMING.DIALOG_WAIT || 2000);
    return;
  }
...
export function ensureProjectDialogOpen(): boolean {
  // V2 Phase 01 Task 01.2: Block dialog click if workspaces not loaded yet
  ...
  for (let i = 0; i < buttons.length; i++) {
    ...
    const isVisible = rect.width > 0 && rect.height > 0 &&
                    window.getComputedStyle(btn as Element).visibility !== 'hidden' &&
                    window.getComputedStyle(btn as Element).display !== 'none';

    if (isVisible) {
      let btnInfo = 'Button: ' + (btn as Element).tagName;
      ...
      try {
        reactClick(btn as Element);
        log('Clicked Project Button successfully — dialog should now be opening', 'success');
        return true;
      } catch(e) {
        log('Click failed on button ' + i + ': ' + (e as Error).message, 'error');
        continue;
      }
    } else {
