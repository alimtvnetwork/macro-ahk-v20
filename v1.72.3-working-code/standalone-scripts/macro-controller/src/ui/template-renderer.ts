 ...
 *
 * Spec: spec/16-standalone-script-assets-pipeline.md §5
...
let _registry: TemplateRegistry = {};
...
export function renderTemplate(name: string, data: Record<string, unknown> = {}): string {
  ...
  if (!entry) {
    throw new Error(`[renderTemplate] Template "${name}" not found. Available: ${Object.keys(_registry).join(', ')}`);
  }
