...
// ============================================
const cfg = window.__MARCO_CONFIG__ || {} as MacroConfig;
...
// ============================================
// Theme: Dual-preset resolver (schema v2)
// ============================================
const themeRoot = window.__MARCO_THEME__ || {} as MacroTheme;

let activeThemeKey = 'dark';
try {
  const saved = localStorage.getItem('marco_theme_preset');
  if (saved && (saved === 'dark' || saved === 'light')) activeThemeKey = saved;
  else if (themeRoot.activePreset) activeThemeKey = themeRoot.activePreset;
} catch(e) { /* ignore */ }

export function resolvePreset(key: string): any {
  if (themeRoot.presets && themeRoot.presets[key]) return themeRoot.presets[key];
  if (themeRoot.colors) return themeRoot;
  return {};
}

const theme = resolvePreset(activeThemeKey);
...
// Activity log state
// ============================================
export let activityLogVisible = false;
export const activityLogLines: any[] = [];
...
export const loopCreditState: Record<string, any> = {
  lastCheckedAt: null,
  perWorkspace: [],
  currentWs: null,
  totalDailyFree: 0,
...
// ============================================
export let loopWsCheckedIds: Record<string, boolean> = {};
export let loopWsLastCheckedIdx = -1;
...
export let LAST_SESSION_BRIDGE_SOURCE = '';
...
export const TOAST_ERROR_AUTO_DISMISS_MS = 15000;
export const toastQueue: any[] = [];
export const toastContainerId = 'marco-toast-container';
export let toastErrorStopTriggered = false;

// ============================================
// Setters for mutable state (needed because ES modules export bindings are read-only to importers)
// ============================================
export function setActivityLogVisible(v: boolean) { activityLogVisible = v; }
export function setLoopWsCheckedIds(v: Record<string, boolean>) { loopWsCheckedIds = v; }
export function setLoopWsLastCheckedIdx(v: number) { loopWsLastCheckedIdx = v; }
export function setLastSessionBridgeSource(v: string) { LAST_SESSION_BRIDGE_SOURCE = v; }
export function setToastErrorStopTriggered(v: boolean) { toastErrorStopTriggered = v; }
