 ...
 * Updated for single-script architecture (macro-looping.js only).
 */
...
// Mock the seed chunks that use ?raw Vite imports (unavailable in test env)
vi.mock("../../src/background/seed-chunks/looping-script-chunk", () => ({
    buildDefaultLoopingScript: () => ({
        id: "default-macro-looping",
        name: "macro-looping.js",
        code: "// mock macro-looping code",
        order: 1,
        isEnabled: true,
        isIife: false,
        configBinding: "default-looping-config",
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
    }),
}));

vi.mock("../../src/background/seed-chunks/looping-config-chunk", () => ({
    buildDefaultLoopingConfig: () => ({
        id: "default-looping-config",
...
describe("Default Project Seeder", () => {
    ...
    it("does NOT skip on update — normalizes project on update too", async () => {
        // The seeder now runs ensureDefaultProjectSingleScript on both install and update
        await simulateInstall("update");

        const snapshot = getMockStoreSnapshot();
        const projects = snapshot["marco_projects"] as any[];
        // On update, it seeds/normalizes the default project
        const hasProjects = Array.isArray(projects) && projects.length > 0;
